-- Boolean features use enabled=true/false, not max_value. The previous
-- canonical function treated enabled=true as max_value=NULL and denied the
-- feature as "No limit configured".
...
BEGIN
  ...
  END IF;
  IF v_allowed AND v_increment>0 AND v_limit IS NOT NULL AND v_limit<>-1 THEN
...
END; $$;
REVOKE ALL ON FUNCTION public.check_feature_limit(uuid,text,integer) FROM PUBLIC, anon, authenticated;
