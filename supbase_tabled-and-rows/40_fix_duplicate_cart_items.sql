...
BEGIN
  -- Consolidate duplicate items in cart to prevent stock lock conflicts
  SELECT jsonb_agg(jsonb_build_object('id', id_text, 'qty', total_qty))
  INTO v_consolidated_items
  FROM (
    SELECT (value->>'id') AS id_text, SUM(COALESCE((value->>'qty')::integer, 0)) AS total_qty
  ...
  IF v_shipping_fee IS NULL THEN
    ...
    INTO v_shipping_fee
    FROM public.shipping_zones
    WHERE store_id = p_store_id AND city_name IN ('?????? ????', 'Other') AND is_active = true
    LIMIT 1;
  END IF;
  ...
  LOOP
    ...
    WHERE id = v_product.id AND store_id = p_store_id AND COALESCE(stock_quantity, stock, 0) >= v_qty;

    IF NOT FOUND THEN
      RAISE EXCEPTION 'Stock update failed for product %', v_product.name;
    END IF;
