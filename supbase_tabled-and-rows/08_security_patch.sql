-- ============================================================
-- RC1 Security Patch (08_security_patch.sql)
-- Scope:
--   - Hardening of create_order_atomic: validate user_id vs auth.uid()
--   - Adjust store_counters RLS policies: grant write capabilities to admins
--   - Make p_store_id in create_order_atomic default to NULL and validate it
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- 1. Hardening of create_order_atomic
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 2. Hardening of store_counters policies
-- ------------------------------------------------------------
DROP POLICY IF EXISTS store_counters_admin_select ON public.store_counters;
DROP POLICY IF EXISTS store_counters_admin_all ON public.store_counters;

CREATE POLICY store_counters_admin_all ON public.store_counters
  FOR ALL
  USING (
