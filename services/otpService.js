...
class OTPService {
  ...
  async sendOTP(phone, store = null) {
    if (!store?.id) {
      throw new Error('OTP requires an active tenant store context');
    }
  ...
  async verifyOTP(phone, code, store = null) {
    if (!store?.id) {
      throw new Error('OTP verification requires an active tenant store context');
    }

    const mutex = this.getPhoneMutex(`${store.id}:${phone}`);
    return mutex.runExclusive(async () => {
      ...
      if (new Date() > new Date(data.expires_at)) {
        await this.deleteOTP(phone, store.id);
